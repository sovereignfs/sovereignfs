...
- **Learning by example:**
  ...
  is the first-party reference set demonstrating each manifest capability
  (`shell: default/minimal/overlay`, `apiProvider`, permissions,
  monetization) using the identical file shape as the template.
- **Installing a plugin into a runtime checkout:** both repo types get
  cloned into `sovereign/plugins/<name>.local` (gitignored, matched via
  pnpm workspace glob) for local/community plugins, or copied to
  `plugins/<slug>` for first-party bundled examples/products declared in
  `sovereign.plugins.json` and installed via
